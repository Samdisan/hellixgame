...
foreach ($phaseData['phases'] ?? [] as $phaseCfg) {
    $phaseOutcomes[$phaseCfg['id'] ?? ''] = $phaseCfg['outcome'] ?? null;
...
$lifeFailActive = false;
foreach ($activePhases as $ap) {
    if (($ap['id'] ?? '') === 'PH_LIFEFAIL' && ($phaseOutcomes['PH_LIFEFAIL'] ?? null) !== 'repaired') {
        $lifeFailActive = true;
        break;
    }
}
...
if ($currentPhaseId && (!$lifeFailActive || $currentPhaseId !== 'PH_LIFEFAIL') && !$isRepairedFailure) {
...
<div class="helix-ambient" aria-hidden="true">
    <div class="helix-ambient__grid"></div>
    <div class="helix-ambient__virus helix-ambient__virus--primary"></div>
    <div class="helix-ambient__virus helix-ambient__virus--secondary"></div>
    <div class="helix-ambient__particles">
        <?php for ($i = 0; $i < 14; $i++): ?>
            <span style="--i: <?php echo $i; ?>;"></span>
        <?php endfor; ?>
    </div>
    <div class="helix-ambient__scanlines"></div>
</div>
